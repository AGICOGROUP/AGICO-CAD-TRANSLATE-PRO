...
The branches have independent importers, placement policies, preflight, final checks and visual receipts; they share only neutral transport and translation batching. In bilingual DWGs, translations may cross drawing lines and use nearby space outside frames; text must not overlap other text. Keep scattered labels locally paired; keep rotated signature translations within their own cells, using horizontal target text when needed. Place prose as same-width, height-adjusting blocks, each source block paired with its own target block rather than consolidated into one shared area, directly below the source as a reading-order continuation, moving to another empty position above, left or right only when the space below is genuinely occupied, preferring spots that leave other views' linework untouched and crossing geometry only as a last resort — this linework preference applies only to prose panels; scattered labels keep text-only clearance and may cross linework as before; first skip existing source/target bilingual labels, paragraphs and tables under the shared skip rule in the bilingual workflow, including bilingual title-block templates even when a filled-in value remains source-only; handle a table in this order: first write each translation into the grid's own aligned target-language column — a bilingual header pair such as 序号/S.No. with BUILDING NAMES beside 建筑名称 — sharing the row number as the authoritative pairing and never moving a translation onto a neighbouring row; then fill only the cells still lacking target text, each inside its own cell; and copy a full target-language table only when it has neither a usable target column nor any target text, with a copied tall table preferring the immediately adjacent horizontal position toward the sheet interior, then the exterior, and a wide table the vertical positions in that order. Never add a suffix beside the original inside a cell even when it has spare room. Follow the grouping and failure rules in the bilingual workflow. Retain original geometry/plot settings and disclose the print-range warning for outside additions. Never switch a sealed job's mode/direction or apply another branch's acceptance. V1 jobs are historical evidence, not reusable V2 jobs.
