...
internal static class Importer
{
    ...
    internal static int Run(JobContext context)
    {
        ...
        BilingualFixedLabelSelection bilingualSelection = new([], new Dictionary<string, string>());
        try
        {
            using (var sideDatabase = new Database(false, true))
            {
                ...
                try
                {
                    ...
                    {
                        LayoutWriteInput[] preliminaryInputs = resolved.Select(write =>
                            new LayoutWriteInput(write.ObjectId, write.Manifest, write.RestoredText,
                                ImportWriteDecision.NeedsLayout(
                                    write.Manifest.PlainText,
                                    translationsById[write.Manifest.RecordId].TranslatedText))).ToArray();
                        CadLayoutBaseline preliminaryBaseline = DrawingTopologyCapture.Capture(
                            sideDatabase,
                            transaction,
                            preliminaryInputs);
                        ...
                        resolved = resolved.Select(write =>
                            bilingualSelection.MixedObjectEnglishTextById.TryGetValue(
                                write.Manifest.RecordId,
                                out string? existingEnglish)
                                ? write with { RestoredText = existingEnglish }
                                : write).ToArray();
                        changed = resolved.Where(write =>
                            suppressed.Contains(write.Manifest.RecordId) ||
                            ImportWriteDecision.NeedsWrite(write.CurrentRawText, write.RestoredText)).ToArray();
            ...
            AtomicFile.WriteUtf8(Path.Combine(context.Config.ArtifactDirectory, "import-summary.json"), JsonSerializer.Serialize(new
            {
                ...
                bilingual = new
                {
                    removedChineseObjects = bilingualSelection.SuppressChineseIds.Count,
                    keptEmbeddedEnglishObjects = bilingualSelection.MixedObjectEnglishTextById.Count
                }
    ...
    private static BilingualFixedLabelSelection SelectBilingualFixedLabels(
        CadLayoutBaseline baseline,
        IReadOnlyList<ResolvedWrite> resolved)
    {
        IReadOnlyDictionary<string, CadLayoutText> topologyByRecord = baseline.Definitions
            .SelectMany(definition => definition.Texts)
            .ToDictionary(text => text.RecordId, StringComparer.Ordinal);
        return BilingualFixedLabelPolicy.Select(resolved
            .Where(write => write.Manifest.ObjectType is "AcDbText" or "AcDbMText")
            .Where(write => topologyByRecord.ContainsKey(write.Manifest.RecordId))
            .Select(write =>
            {
                CadLayoutText topology = topologyByRecord[write.Manifest.RecordId];
                return new BilingualFixedLabelSample(
                    write.Manifest.RecordId,
                    write.Manifest.RawText,
                    write.RestoredText,
                    write.Manifest.ObjectType,
                    topology.DefinitionName,
    ...
    private static LayoutAdjustment[] ApplyGlobalCorrectionPass(
        string temporaryOutput,
        string extension,
        CadLayoutBaseline baseline,
        LayoutOptimizationResult optimization,
    ...
    {
        string correctionOutput = CreateSiblingTemporaryPath(temporaryOutput);
        LayoutAdjustment[] corrections = [];
        try
        {
            using (var database = new Database(false, true))
            {
                ReadWorkingDrawing(database, temporaryOutput, Path.GetDirectoryName(temporaryOutput)!);
                Database originalWorkingDatabase = HostApplicationServices.WorkingDatabase;
                try
                {
                    HostApplicationServices.WorkingDatabase = database;
                    using Transaction transaction = database.TransactionManager.StartTransaction();
                    corrections = GlobalCollisionCorrection.Apply(
                        database,
                        transaction,
                        baseline,
                        optimization,
                    ...
                    {
                        SaveTemporaryOutput(database, correctionOutput, extension);
                    }
                }
                finally
                {
            ...
            if (corrections.Length > 0)
            {
                File.Move(correctionOutput, temporaryOutput, overwrite: true);
            }
        ...
        {
            if (File.Exists(correctionOutput)) File.Delete(correctionOutput);
        }
    }
